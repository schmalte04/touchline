...
class CSVMCPServer {
    ...
    async handleSearchCSV(args) {
        ...
        if (column) {
            ...
            results = this.csvData.filter(row => row[column]?.toLowerCase().includes(query));
        }
        else {
            results = this.csvData.filter(row => Object.values(row).some(value => value?.toLowerCase().includes(query)));
        }
        return {
            content: [
                {
    ...
    async handleFilterCSV(args) {
        ...
        const results = this.csvData.filter(row => row[column]?.toLowerCase().includes(value));
        return {
            content: [
                {
                    type: 'text',
